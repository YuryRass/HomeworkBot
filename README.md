# Описание системы
**HomeworkBot** - программная система, разработанная в рамках курса [@Грокаем_Python_через_разработку_проекта](https://stepik.org/course/138391)
на платформе [@Stepik](https://stepik.org). Программа предназначена для автоматизированной проверки
лабораторных или домашних работ студентов, являющихся участниками определенного телеграм-чата.

При разработке программного средства **HomeworkBot** дополнительно были выполнены следующие задачи: \
1. Реализация Telegram бота с использованием библиотеки Aiogram:3.1.1
2. Примение Docker Volume для получения результатов тестирования из docker-контейнера
3. Миграция СУБД SQLite на PostgreSQL
4. Асинхронная реализация SQLAlchemy 2.0
5. Покрытие кода комментариями
...
организованной с использованием PostgreSQL.
...
выгружать весь архив с работами за семестр);
...
1. Преподаватель  (имеет возможность формировать отчеты
по группам, у которых ведет занятия, выгружать ответы для проверки
банить и снимать бан со студента);
1. Студент (загружает работы, может просматривать свою
успеваемость).
...
Далее приведена структура базы данных, выполнняющей роль очереди
...
> Зависимости системы: SQLAlchemy, SQLAlchemy-Utils, aiohttp, pydantic, asyncpg \
> AIOgram, asyncinit, python-on-whales, openpyxl, python-dotenv
...
`git clone https://github.com/YuryRass/HomeworkBot.git`

Затем установите необходимые зависимости из файла requirements.txt:

`pip3 install -r requirements.txt`

Для создания и запуска БД PostgreSQL в docker-контейнере выполните следующую команду:

`docker-compose -f postgresql/docker-compose.yaml up --build`

Для подключения к БД PostgreSQL на host-системе выполните команду:

`sudo -u postgres psql`

> postgres - имя пользователя в PostgreSQL по-умолчанию

После подключения к БД выполните следующие команды для создания баз данных:

* `create database <MAIN_DB>;`
* `create database <QUEUE_DB>;`

Создайте и заполните `.env` файл. \
Если будете запускать БД в docker контейнере, то переменной **PSQL_PORT** присвойте значение **HOST_PORT**, взятое из файла `./postgresql/.env`.
Если вы планируете удаленную конфигурацию,
...
* **Добавить дисциплину**
  ...
  >   "short_name": "ТМП-2",
  >   "path_to_test": "_disciplines/tmp2/test",
  >   "path_to_answer": "_disciplines/tmp2/answer",
  >   "language": "python",
  >   "works": [
  >     {
  >       "number": 1,
  ...
  "disciplines": [
    {
      "full_name": "Технологии и методы программирования",
      "short_name": "ТМП",
      "path_to_test": "_disciplines/tmp/test",
      "path_to_answer": "_disciplines/tmp/answer",
      "language": "python",
      "works": [
        {
          "number": 1,
