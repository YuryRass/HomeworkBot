"""Модуль реализует подистему тестирования"""
...
class TaskProcessing:
    ...
    def __init__(
        ...
        """
        :param temp_folder_path: путь до временной директории,
        где будут формироваться каталоги для создания docker-контейнеров
        :param docker_amount_restriction: ограничение на количество
        одновременно работающих контейнеров
        """
...
def _run_prepare_docker(record: QueueIn, temp_folder_path: Path) -> None:
    ...
    :param record: запись из промежуточной БД, с данными
    по загруженным ответам студента
    :param temp_folder_path: путь до временной директории,
    где будут формироваться каталоги для создания docker-контейнеров
    ...
    if folder_builder.has_rejected_files():
        rejected_crud.add_record(
            ...
            TestRejectedFiles(
                ...
                description='Имя файла(-ов) не соответствуют' +
    ...
    # проверка на ключевые слова в ответах студентов
    ...
    if keywords_controller.has_rejected_files():
        rejected_crud.add_record(
            ...
            TestRejectedFiles(
                ...
                ' ключевые слова, либо не используются ' +
